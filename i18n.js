...
const STR = {
  ...
  fr: {
    ...
    coach4t: 'FIX',
    coach4b: 'SNAP les kits verts FIX. Charge 3/3, puis appuie FIX (ordi : Espace) pour réparer +1 coque. Portail : 4 SNAPs relic → SNAP le portail vers les Mondes alternatifs (orbes + débris, 30 s).',
    ...
    hintBonus: (mult) => `MONDES ALTERNATIFS · orbes ×${Number(mult).toFixed(2)} · 3+ = +1 vie · 30s · débris dangereux`,
    bonusTimer: (s) => `ALT ${s}s`,
    bonusEnter: (tier, mult) => `MONDES ALT T${tier || 1} · ×${Number(mult ?? 1.5).toFixed(2)}`,
