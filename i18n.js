...
const STR = {
  en: {
    ...
    coach4t: 'FIX',
    coach4b: 'SNAP green FIX kits in the field. Charge 3/3, then tap FIX (desktop: Space) to repair +1 hull. Portal: 4 relic SNAPs → SNAP the portal into the alternate world (hostile orbs + debris, 30s).',
    fixCharged: 'FIX CHARGED',
    fixReady: 'FIX ready — repair hull',
    fixNeed: (n, need) => `FIX ${n}/${need}`,
    fix: 'FIX',
    hullFull: 'HULL FULL',
    fixHeal: '+1 HULL',
    kitGet: 'FIX KIT',
    ...
    hull: 'HULL',
    hintPortal: 'PORTAL open · SNAP to enter alternate world',
    hintBonus: (mult) => `ALTERNATE WORLD · orbs ×${Number(mult).toFixed(2)} · 3+ = +1 life · 30s · debris hurts`,
    bonusTimer: (s) => `ALT ${s}s`,
    bonusEnter: (tier, mult) => `ALT WORLD T${tier || 1} · ×${Number(mult ?? 1.5).toFixed(2)}`,
    bonusClear: 'ALT CLEAR',
    bonusLife: '+1 HULL',
  ...
  fr: {
    ...
    coach4t: 'FIX',
    coach4b: 'SNAP les kits verts FIX. Charge 3/3, puis appuie FIX (ordi : Espace) pour réparer +1 coque. Portail : 4 SNAPs relic → SNAP le portail vers le monde alternatif (orbes + débris, 30 s).',
    fixCharged: 'FIX CHARGÉ',
    fixReady: 'FIX prêt — répare la coque',
    fixNeed: (n, need) => `FIX ${n}/${need}`,
    fix: 'FIX',
    hullFull: 'COQUE PLEINE',
    fixHeal: '+1 COQUE',
    kitGet: 'KIT FIX',
    mute: 'Muet',
    ...
    hull: 'COQUE',
    hintPortal: 'PORTAIL ouvert · SNAP pour le monde alternatif',
    hintBonus: (mult) => `MONDE ALTERNATIF · orbes ×${Number(mult).toFixed(2)} · 3+ = +1 vie · 30s · débris dangereux`,
    bonusTimer: (s) => `ALT ${s}s`,
    bonusEnter: (tier, mult) => `MONDE ALT T${tier || 1} · ×${Number(mult ?? 1.5).toFixed(2)}`,
    bonusClear: 'ALT RÉUSSI',
    bonusLife: '+1 COQUE',
  ...
  es: {
    ...
    coach4t: 'FIX',
    coach4b: 'SNAP kits verdes FIX. Carga 3/3, luego toca FIX (PC: Espacio) para reparar +1 casco. Portal: 4 SNAPs relic → SNAP el portal al mundo alternativo (orbes + escombros, 30s).',
    fixCharged: 'FIX CARGADO',
    fixReady: 'FIX listo — repara el casco',
    fixNeed: (n, need) => `FIX ${n}/${need}`,
    fix: 'FIX',
    hullFull: 'CASCO LLENO',
    fixHeal: '+1 CASCO',
    kitGet: 'KIT FIX',
    mute: 'Mudo',
    ...
    hull: 'CASCO',
    hintPortal: 'PORTAL abierto · SNAP al mundo alternativo',
    hintBonus: (mult) => `MUNDO ALTERNATIVO · orbes ×${Number(mult).toFixed(2)} · 3+ = +1 vida · 30s · escombros dañan`,
    bonusTimer: (s) => `ALT ${s}s`,
    bonusEnter: (tier, mult) => `MUNDO ALT T${tier || 1} · ×${Number(mult ?? 1.5).toFixed(2)}`,
    bonusClear: 'ALT COMPLETO',
    bonusLife: '+1 CASCO',
