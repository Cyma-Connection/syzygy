...
const STR = {
  en: {
    ...
    coach4t: 'FIX',
    coach4b: 'SNAP green FIX kits in the field. Charge 3/3, then tap FIX (desktop: Space) to repair +1 hull. Portal: 4 relic SNAPs → SNAP the portal into the Alternate worlds (hostile orbs + debris, 30s).',
    ...
    hintBonus: (mult) => `ALTERNATE WORLDS · orbs ×${Number(mult).toFixed(2)} · 3+ = +1 life · 30s · debris hurts`,
    bonusTimer: (s) => `ALT ${s}s`,
    bonusEnter: (tier, mult) => `ALT WORLDS T${tier || 1} · ×${Number(mult ?? 1.5).toFixed(2)}`,
  ...
  es: {
    ...
    coach4t: 'FIX',
    coach4b: 'SNAP kits verdes FIX. Carga 3/3, luego toca FIX (PC: Espacio) para reparar +1 casco. Portal: 4 SNAPs relic → SNAP el portal a los Mundos alternativos (orbes + escombros, 30s).',
    ...
    hintBonus: (mult) => `MUNDOS ALTERNATIVOS · orbes ×${Number(mult).toFixed(2)} · 3+ = +1 vida · 30s · escombros dañan`,
    bonusTimer: (s) => `ALT ${s}s`,
    bonusEnter: (tier, mult) => `MUNDOS ALT T${tier || 1} · ×${Number(mult ?? 1.5).toFixed(2)}`,
