export default function (THREE) {
  ...
  const oculus = new THREE.Mesh(
    ...
    (() => { const m = new THREE.MeshStandardMaterial({{ color: obs, roughness: 0.25, metalness: 0.3, emissive: cold, emissiveIntensity: 0.15 }}); m.name = "stone"; return m; })()
  );
  ...
  for (let i = 0; i < 3; i++) {
    const thr = new THREE.Mesh(
      ...
      (() => { const m = new THREE.MeshStandardMaterial({{ color: bone, roughness: 0.55, emissive: 0xe8a04a, emissiveIntensity: 0.12 }}); m.name = "metal"; return m; })()
    );
